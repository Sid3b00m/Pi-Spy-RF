...
# Added: GNU Radio integration for improved FM demodulation, especially for HackRF APT decoding
#
# Limitations:
...
install_deps() {
    ...
    sudo apt update
    sudo apt install -y git python3 python3-pip sqlite3 libusb-1.0-0-dev rtl-sdr sox multimon-ng kismet pandoc unzip curl gnuradio gnuradio-dev gr-osmosdr
    # For HackRF support
    if [ "$1" == "--hackrf" ]; then
        sudo apt install -y hackrf libhackrf-dev gr-hackrf
    fi
...
# GNU Radio imports for FM demodulation
...
class APTDemod(gr.top_block):
    """GNU Radio flowgraph for APT FM demodulation to WAV"""
    def __init__(self, device_str, freq, gain, wav_file, samp_rate=2000000, quad_rate=200000, audio_rate=41600):
        super().__init__()

        # Parameters
        tune_freq = freq + 25000  # Offset to avoid DC spike
        decim = int(samp_rate / quad_rate)
        max_dev = 17000  # APT deviation ~17 kHz
        tau = 0.0  # No de-emphasis for APT

        # Source
        self.src = osmosdr.source(args=device_str)
        self.src.set_sample_rate(samp_rate)
        self.src.set_center_freq(tune_freq)
        self.src.set_gain(gain, 'rf')
        self.src.set_gain(20, 'if')  # Adjust as needed
        self.src.set_gain(20, 'bb')  # Adjust as needed

        # Frequency translating FIR filter (shift 0, lowpass, decimate)
        taps = firdes.low_pass(1.0, samp_rate, 20000, 10000, window=firdes.window.WIN_HAMMING, beta=6.76)
        self.xlate = filter.freq_xlating_fir_filter_ccc(decim, taps, 0, samp_rate)

        # NBFM Receive
        self.nbfm = analog.nbfm_receive(audio_rate=audio_rate, quad_rate=quad_rate, tau=tau, max_dev=max_dev)

        # WAV Sink
        self.sink = blocks.wavfile_sink(wav_file, 1, audio_rate, blocks.FORMAT_WAV, blocks.FORMAT_PCM_16, False)

        # Connections
        self.connect(self.src, self.xlate)
        self.connect(self.xlate, self.nbfm)
        self.connect(self.nbfm, self.sink)
...
class RadioScanner:
    def __init__(self, config_path, use_hackrf=False):
        ...
        self.device_str = "hackrf=0" if use_hackrf else "rtl=0"
    ...
    def decode_apt(self, freq, record_duration=60):
        """Decode NOAA APT satellite image from signal using GNU Radio for demod"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        wav_file = f"noaa_apt_{timestamp}_{int(freq/1e3)}kHz.wav"
        png_file = wav_file.replace('.wav', '.png')
        
        logger.info(f"Recording APT signal at {freq/1e6} MHz for {record_duration} seconds using GNU Radio...")
        
        tb = APTDemod(self.device_str, freq, self.gain, wav_file)
        tb.start()
